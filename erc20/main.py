import os
from erc20_wallet import ERC20Wallet
    ...
    while True:
        try:
            ...
            if command == "/info":
                ...
                for token in wallet.supported_tokens.keys():
                    print(f"  - {token}")
                
                token_name = input("\nEnter token name: ").strip().upper()
                
                if token_name not in wallet.supported_tokens:
                    print(f"Token {token_name} not supported!")
                    continue
