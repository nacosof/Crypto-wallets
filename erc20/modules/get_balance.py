...
def get_main_coin_balance(address):
    ...
    try:
        ...
        if not web3.is_connected():
            return {"balance": 0, "error": "Cannot connect to Ethereum network"}
...
def get_all_balances(address):
    ...
    eth_result = get_main_coin_balance(address)
    balances["ETH"] = eth_result["balance"]
    if eth_result["error"]:
        errors["ETH"] = eth_result["error"]
