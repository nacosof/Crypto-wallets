...
from config import TESTNET, MAINNET_RPC, TESTNET_RPC, SUPPORTED_TOKENS, POLYGON_CHAIN_ID, MUMBAI_CHAIN_ID, DEFAULT_GAS_LIMIT, TOKEN_GAS_LIMIT
...
def send_matic(from_address, private_key, to_address, amount):
    try:
        rpc_url = TESTNET_RPC if TESTNET else MAINNET_RPC
        web3 = Web3(Web3.HTTPProvider(rpc_url))
        
        if not web3.is_connected():
            return {"success": False, "error": "Cannot connect to Polygon network"}
        ...
        tx = {
            ...
            'gasPrice': gas_price,
            'chainId': MUMBAI_CHAIN_ID if TESTNET else POLYGON_CHAIN_ID
        }
        ...
        return {"success": False, "error": f"Send MATIC error: {str(e)}"}
...
def send_token(from_address, private_key, to_address, amount, token_name):
    ...
    if token_name == 'MATIC':
        return send_matic(from_address, private_key, to_address, amount)
    else:
